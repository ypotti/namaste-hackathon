...
describe("GameRenderer registry",()=>{
 ...
 it("supports open-ended sandbox exploration",()=>{render(<GameRenderer spec={projectileFixture as any} mode="sandbox"/>);expect(screen.getByText("Live exploration")).toBeTruthy();expect(screen.getByRole("button",{name:"Reset variables"})).toBeTruthy();expect(screen.queryByRole("button",{name:"Check answer"})).toBeNull()});
});
