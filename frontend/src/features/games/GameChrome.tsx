...
export function GameChrome({spec,children,onCheck,onReset,feedback,won}:{spec:GameSpecV1;children:ReactNode;onCheck:()=>void;onReset:()=>void;feedback?:string;won:boolean}){
 ...
 </div>{feedback&&<p role="status" aria-live="polite"><strong>{won?spec.learning.principle:"Keep experimenting"}</strong> — {feedback}</p>}{hint&&<p role="note">{spec.learning.hint}</p>}</>:<div className="sandbox-bar"><span><strong>Live exploration</strong> — adjust the variables and watch the model respond in real time.</span><button type="button" onClick={onReset}>Reset variables</button></div>}</section>
}
