...
export function App() {
  ...
  return <main className={`app-${view}`}>
    ...
    <div className="workspace" id="main"><aside className="lesson"><div className="index">01 <i /></div><p className="kicker">CONCEPT LOADED</p><h1>{spec.concept}</h1><p><strong>{spec.learning.principle}.</strong> {spec.learning.explanation}</p><div className="equation">{spec.game_type.replaceAll("_", " ")}</div><div className="mission"><span>YOUR MISSION</span><p>{spec.instructions}</p></div><div className="verified"><span>✓</span><div><strong>Solver verified</strong><small>This challenge is proven winnable</small></div></div></aside>
      <section className="game creator-preview" aria-labelledby="game-title"><header><div><p>CREATOR PREVIEW · {spec.eyebrow}</p><h2 id="game-title">{spec.title}</h2></div><button className="open-learner" onClick={()=>setView("learner")}>Open learner view</button><div className="meta"><span>{source === "api" ? "GAME READY" : "DEMO READY"}</span><div>{spec.difficulty.toUpperCase()}</div></div></header><iframe key={gameContentUrl(gameId)} title={`${spec.title} creator preview`} src={gameContentUrl(gameId)} sandbox="allow-scripts" /></section>
    </div></>}
    ...
    <footer><span>LESSONS TURNED INTO GAMES · KNOWLEDGE THAT STICKS</span><span>HTML ARTIFACT · GAME SPEC v{spec.schema_version}</span></footer>
  </main>;
}
